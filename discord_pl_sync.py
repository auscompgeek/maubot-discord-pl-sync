...
class DiscordRolePLSync(Plugin):
    ...
    async def handle_membership(self, evt: StateEvent) -> None:
        ...
        await self._update_member(room_id, mxid, evt.content)

    async def _update_member(
        self, room_id: RoomID, mxid: UserID, member: MemberStateEventContent,
    ):
        ...
        for role in sorted(user_roles, key=lambda role: -role["position"]):
            ...
            if role_pl is not None:
                self.log.debug("%s in %s has role %s", mxid, room_id, role["name"])
